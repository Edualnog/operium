...
BEGIN
    ...
    IF v_org_id IS NOT NULL AND v_role IS NOT NULL THEN
        -- Create operium_profile if not exists
        IF NOT EXISTS (SELECT 1 FROM operium_profiles WHERE user_id = NEW.id) THEN
            INSERT INTO operium_profiles (user_id, org_id, role, active, onboarding_complete)
            VALUES (NEW.id, v_org_id, v_role, true, false);
        END IF;
